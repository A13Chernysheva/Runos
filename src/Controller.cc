...
#define FILE_FOR_TABLES "dpid_and_t.txt"
#define FILE_FOR_APPS "apps_and_fields.txt"
                     ...
                     , public maple::Flow
{
   ...
   class DecisionCompiler : public boost::static_visitor<void> {
    ...
    ActionList actions()
    {
        ...
        for (uint8_t i = 0; i < m_decision.size(); i++) {
            boost::apply_visitor(DecisionCompiler(ret, m_flood), m_decision.data(i));
        }
        return ret;
    }
    ...
    void install()
    {
        ...
        if (m_decision.idle_timeout() <= Decision::duration::zero()) {
            ...
            if (GoToTable) {
                fm.add_instruction(new of13::GoToTable(m_table + 1));
            }
...
public:
    ...
    void installTable(uint8_t table) {
        m_table = table;
    }
...
struct SwitchScope {
    ...
    std::vector<FlowImplPtr> miss;
    std::vector<MapleBackend> backend;
    std::vector<maple::Runtime<DecisionImpl, FlowImpl>> runtime;
        ...
        , flood(std::bind(_flood, dpid))
    {
        ...
        //--------
        handler_table = InitPipeline(pipeline_factory, pipeline, dpid);
        //--------

        backend.reserve(handler_table);
        miss.reserve(handler_table);
        runtime.reserve(handler_table);

        for (uint8_t i = 0; i < handler_table; i++) {

            backend.emplace_back(connection, i);
            miss.push_back(FlowImplPtr(std::make_shared<FlowImpl>(connection, i, std::bind(_flood, 0))));
            runtime.emplace_back(std::bind(&SwitchScope::policy, this, _1, _2, _3), backend[i], miss[i]);
        ...
        for (uint8_t i = 0; i < handler_table; i++){
            miss[i]->decision( DecisionImpl{}.inspect(128) ); // TODO: unhardcode
        }

        installTableMissRule();
    }

    uint8_t SimpleInitPipeline(PacketMissPipelineFactory const& pipeline_factory,
                      PacketMissPipeline &pipeline) {
        PacketMissPipelineTable first, second;
        for (uint8_t i = 0; i < pipeline_factory.size() - 1; i++ ) {
            first.emplace_back(pipeline_factory[i].first,
                                  std::move(pipeline_factory[i].second(connection)));
        }
        pipeline.emplace_back(first);
        second.emplace_back(pipeline_factory[pipeline_factory.size() - 1].first,
                                  std::move(pipeline_factory[pipeline_factory.size() - 1].second(connection)));
        pipeline.emplace_back(second);
        return pipeline.size();
    }

    uint8_t InitPipeline(PacketMissPipelineFactory const& pipeline_factory,
                      PacketMissPipeline &pipeline, uint64_t dpid) {
        uint8_t N = pipeline_factory.size(), T;
        ifstream file(FILE_FOR_TABLES);
        string sdpid('#' + to_string(dpid)), word;
        if (file.is_open()) {
            while (file >> word) {
                if (word == sdpid) {
                    file >> word;
        ...
        if (N <= T) {
            PacketMissPipelineTable one;
            for (uint8_t i = 0; i < pipeline_factory.size(); i++ ) {
                one.emplace_back(pipeline_factory[i].first,
                                  std::move(pipeline_factory[i].second(connection)));
                pipeline.emplace_back(one);
                one.clear();
            }
        }
        else {
            Pipeline p(FILE_FOR_APPS);
            Combiner c;
            bool run = true;
            while(run) {
                for(uint8_t k = 2; k <= N - (T - 1); k++) {
                    c.putActions(p.symDifAllByk(k));
                }
                c.sort();
                while(true) {
                    OneAction act = c.getAction();
                    p.applyAction(act.position, act.length);
                    N--;
                    if (N <= T) {
                        run = false;
                        break;
            ...
            pipeline = p.getPipeline(pipeline_factory, connection);
        }
        return pipeline.size();
    }
    ...
    DecisionImpl policy(Packet& pkt, FlowImplPtr& flow, PacketMissPipelineTable partpipeline) const
    {
        DecisionImpl ret = DecisionImpl{};
        for (auto& handler: partpipeline) {
            try {
                ret = (DecisionImpl&&)(handler.second(pkt, flow, ret));
                if (ret.base().return_) {
                    flow->GoToTable = false;
                    return ret;
                }
                flow->GoToTable = true;
    ...
    bool isTableMiss(of13::PacketIn& pi, int table) const
    {
        if (pi.reason() == of13::OFPR_NO_MATCH)
            return true;
        if (pi.reason() == of13::OFPR_ACTION && pi.cookie() == miss[table]->cookie())
            return true;
        return false;
    }
    ...
    {
        for (uint8_t i = 0; i < handler_table; i++) {
            backend[i].remove(oxm::field_set{});
            backend[i].barrier();
...
public:
    ...
    void connection_callback(OFConnection *ofconn, OFConnection::Event type) override
    {
        ...
        if (type == OFConnection::EVENT_STARTED) {
            ...
            if (ctx) {
                ...
                for(auto& runtime_i: ctx->runtime) {
                    runtime_i.invalidate();
                }
                ctx->flows.clear();
            }
        }
    ...
    void invalidateTraceTree()
    {
        ...
        for (auto i =  switches.begin(); i != switches.end(); i++){
            for(auto& runtime_i: i->second.runtime) {
                runtime_i.invalidate();
            }
            for(auto& backend_i: i->second.backend) {
                backend_i.remove(oxm::field_set{});
            }
        }
    }
};
...
void SwitchScope::processPacketIn(of13::PacketIn& pi)
{
    ...
    for(uint8_t i = pi.table_id(); i < handler_table; i++) {

        DVLOG(10) << "Packet-in on switch " << connection->dpid()
              << (isTableMiss(pi, i) ? " (miss)" : " (inspect)");

        // Find flow in the trace tree
        std::shared_ptr<FlowImpl> flow = runtime[i](pkt);

        DVLOG(30) << "flow cookie is : " << std::setbase(16) << flow->cookie() << " packet cookie : " << pi.cookie();
        // Delete flow if it doesn't found or expired
        if (flow == miss[i] || flow->state() == Flow::State::Expired) {
            DVLOG(30) << "hit on  mapple-barrier rule. Cookie : " << std::setbase(16) << miss[i]->cookie();
            flow = std::make_shared<FlowImpl>(connection, i, flood);
        ...
        switch (flow->state()) {
            ...
            case Flow::State::Idle:
            {
                ...
                flow->installTable(i);
                runtime[i].augment(mpkt, flow, pipeline[i]);
                if (i == (handler_table-1)) {
                    flow->GoToTable = false;
                }
            ...
            {
                DLOG_IF(ERROR, isTableMiss(pi, i))
                ...
                if (not isTableMiss(pi, i)){
                    flow->decision(policy(pkt, flow, pipeline[i]));
                } else {
                    flow->wakeup();
                }
