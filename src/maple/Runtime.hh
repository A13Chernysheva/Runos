...
class Runtime {
    ...
    using Policy = std::function<Decision(Packet& pkt, FlowPtr& flow, PacketMissPipelineTable partpipeline)>;
...
public:
    ...
    FlowPtr augment(Packet& pkt, FlowPtr& flow, PacketMissPipelineTable partpipeline)
    {
        ...
        try {
            flow->decision(policy(tpkt, flow, partpipeline));
